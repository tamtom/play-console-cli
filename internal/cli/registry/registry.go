...
func NewCatalog(version string, rt *cliruntime.Runtime) *Catalog {
	...
	catalog.specs = []CommandSpec{
		...
		commandSpecWith("onetime-products", "Manage one-time products through the current monetization API.", "android-publisher-api", []string{"app.one_time_products"}, onetimeproducts.OneTimeProductsCommand),
		commandSpecWith("onetimeproducts", "DEPRECATED: use `gplay onetime-products` instead.", "android-publisher-api", nil, onetimeproducts.LegacyOneTimeProductsCommand),
