...
func NewCatalog(version string, rt *cliruntime.Runtime) *Catalog {
	...
	catalog.specs = []CommandSpec{
		...
		commandSpec("capabilities", "Show policy-aware Google Play workflow capabilities.", capabilities.Command),
		...
		commandSpecWith("integrity", "Decode Play Integrity tokens and manage restricted Device Recall state.", "play-integrity-api", []string{"app.play_integrity"}, integrity.Command),
		...
		commandSpecWith("insights", "Compare trends from official Google Play report exports.", "local-official-reports", []string{"app.insights"}, insights.Command),
