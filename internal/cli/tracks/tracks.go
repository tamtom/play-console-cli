...
func CreateCommand() *ffcli.Command {
	fs := flag.NewFlagSet("tracks create", flag.ExitOnError)
	packageName := fs.String("package", "", "Package name (applicationId)")
	editID := fs.String("edit", "", "Edit ID")
	track := fs.String("track", "", "Track name to create")
	outputFlag := fs.String("output", "json", "Output format: json (default), table, markdown")
	pretty := fs.Bool("pretty", false, "Pretty-print JSON output")

	return &ffcli.Command{
		Name:       "create",
		ShortUsage: "gplay tracks create --package <name> --edit <id> --track <name>",
		ShortHelp:  "Create a new custom track.",
		LongHelp: `Create a new custom track.
...
production, beta, alpha, and internal tracks.`,
		...
		Exec: func(ctx context.Context, args []string) error {
			...
			trackObj := &androidpublisher.TrackConfig{
				Track: *track,
			}
			...
			resp, err := service.API.Edits.Tracks.Create(pkg, *editID, trackObj).Context(ctx).Do()
			if err != nil {
				return err
			}
			return shared.PrintOutput(resp, *outputFlag, *pretty)
